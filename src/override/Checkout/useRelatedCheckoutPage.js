...
const relatedWishlistConfigQuery = gql`
query($location: String) {
  mpARPCheckoutPage(filter:{location:{eq: $location}}) {
    items {
      actions {
        aggregator
        attribute
...
export const useRelatedCheckout = (props) => {
    const location = props ? props.location : null;

    const {data: configData, loading, error} = useQuery(relatedWishlistConfigQuery, {
        variables: {
            location: location
        },
    ...
    const shouldRenderRelatedProduct = (
        !!configData
        && !!configData.mpARPCheckoutPage
        && !!configData.mpARPCheckoutPage.items
        && (configData.mpARPCheckoutPage.items.length > 0)
    )
    ...
    const config = useMemo(() => normalizeRule(
        shouldRenderRelatedProduct ? configData.mpARPCheckoutPage.items : null,
        {
            location: location,
